...
export const ConfirmationDialog: React.FC<ConfirmationDialogProps> = ({
  ...
  const iconMap = {
    danger: <Trash2 className="w-5.5 h-5.5 text-rose-600 dark:text-rose-400" />,
  ...
  const bgIconMap = {
    ...
    info: 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-100 dark:border-indigo-900/40',
  };

  const confirmBtnClass = {
    danger: 'bg-rose-600 hover:bg-rose-700 text-white focus:ring-rose-500 shadow-rose-500/10',
    warning: 'bg-amber-600 hover:bg-amber-700 text-white focus:ring-amber-500 shadow-amber-500/10',
    success: 'bg-emerald-600 hover:bg-emerald-700 text-white focus:ring-emerald-500 shadow-emerald-500/10',
    info: 'bg-indigo-600 hover:bg-indigo-700 text-white focus:ring-indigo-500 shadow-indigo-500/10',
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 w-full max-w-md p-6 shadow-2xl relative z-55 overflow-hidden font-sans"
          >
            ...
            <div className="flex gap-2.5 mt-6 justify-end pt-3 border-t border-slate-100 dark:border-slate-800/80">
              <button
                ...
                onClick={onClose}
                disabled={isLoading}
                className="py-2.5 px-4 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold rounded-xl text-xs transition cursor-pointer border border-slate-200 dark:border-slate-700 disabled:opacity-50"
              >
                {cancelText}
              </button>
              <button
